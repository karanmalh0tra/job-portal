...
class AdminService extends MySql
{
	...
	function viewCompany()
	{
		...
		try
		{
			$query = "SELECT * FROM companies where archive='n'";
			$data = $this->ExecuteQuery($query, "select");
			$this->commitTransaction();
		}
		catch(Exception $e)
	...
	function viewUser()
	{
		...
		try
		{
			$query = "SELECT * FROM users where archive='n'";
			$data = $this->ExecuteQuery($query, "select");
			$this->commitTransaction();
		}
		catch(Exception $e)
	...
	function getCompanyById($companyId)
	{
		...
		try
		{
			$query = "SELECT * FROM companies WHERE company_id='".$companyId."' and archive='n'";
			$data = $this->ExecuteQuery($query, "select");
			$admin = $data[0];
			$this->commitTransaction();
		}
	...
	function getUserById($userId)
	{
		...
		try
		{
			$query = "SELECT * FROM users WHERE user_id='".$userId."' and archive='n'";
			$data = $this->ExecuteQuery($query, "select");
			$admin = $data[0];
			$this->commitTransaction();
		}
	...
	function updateCompany($post, $companyId)
	{
		...
		try
		{
			...
			//INSERT INTO admin (admin_name, admin_mobile, admin_email, password,role)
			$query = "UPDATE companies SET company_name = '$name', company_contact='$mobile', company_email='$email', company_address='$address', company_contact_person='$person' where company_id='".$companyId."'";
			$this->ExecuteQuery($query, "update");
	...
	function updateUser($post, $userId)
	{
		...
		{
			$name = $post['name'];
			$resume = mysql_real_escape_string($post['resume']);
			$mobile = $post['mobile'];
			$email = $post['emailId'];
			$address = $post['address'];
			$dob= $post['dob'];
			...
			//INSERT INTO admin (admin_name, admin_mobile, admin_email, password,role)
			$query = "UPDATE users SET user_name = '$name', resume_headline='$resume', user_mobile='$mobile', user_email='$email', address='$address', date_of_birth='$dob' where user_id='".$userId."'";
			$this->ExecuteQuery($query, "update");
	...
	function deleteCompany($companyId)
	{
		...
		try
		{
			$query = "UPDATE companies set archive = 'y' WHERE company_id='".$companyId."'";
			$data = $this->ExecuteQuery($query, "update");
			if(!empty($data)){
				$delete = 1;
			}
	...
	function deleteUser($userId)
	{
		...
		try
		{
			$query = "UPDATE users set archive = 'y' WHERE user_id='".$userId."'";
			$data = $this->ExecuteQuery($query, "update");
			if(!empty($data)){
				$delete = 1;
			}
